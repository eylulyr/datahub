...
def _detect_mode() -> str:
    """Return 'acryl' if `acryl-main` ref exists in this repo, else 'oss'.
    ...
    signal stable across local renames and detached HEADs.
    """
    try:
        subprocess.check_output(
            ["git", "rev-parse", "--verify", "--quiet", "acryl-main"],
            cwd=REPO_ROOT,
            stderr=subprocess.DEVNULL,
        )
        return "acryl"
...
def _default_head(mode: Optional[str] = None) -> str:
    ...
    if mode is None:
        mode = _detect_mode()
    return "acryl-main" if mode == "acryl" else "master"
